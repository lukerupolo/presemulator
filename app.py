        ...
        with st.spinner("Assembling your new presentation... This may take a moment."):
            try:
                ...
                if len(st.session_state.structure) > len(new_prs.slides):
                    st.error(f"Error: Your defined structure has {len(st.session_state.structure)} steps, but the template only has {len(new_prs.slides)} slides. Please adjust your structure.")
                    st.stop()
                ...
                for i, step in enumerate(st.session_state.structure):
                    ...
                    if action == "Copy from GTM (as is)":
                        ...
                        if src_slide:
                            build_plan.append({"action": "copy", "dest_slide": dest_slide, "src_slide": src_slide, "keyword": keyword})
                        else:
                        ...
                        if content_slide:
                            ...
                            st.warning(f"AI could not find content for '{keyword}' in GTM Deck. Leaving template slide {i+1} as is.")
                ...
                # Remove any slides from the end of the template that were not in the structure
                num_to_delete = len(new_prs.slides) - len(build_plan)
                if num_to_delete > 0:
                    for i in range(len(new_prs.slides) - 1, len(build_plan) - 1, -1):
